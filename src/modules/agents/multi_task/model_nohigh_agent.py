...
class MODELNHAgent(nn.Module):
    ...
    def forward_action(self, inputs, discr_h, hidden_state_dec, hidden_state_plan, task, 
                       mask=False, t=0, actions=None):
        h_plan = hidden_state_plan
        act, h_dec, cls_out = self.decoder(inputs, discr_h, hidden_state_dec, task, mask, actions)
    ...
    def forward(self, inputs, hidden_state_plan, hidden_state_dec, hidden_state_dis, t, task, skill, 
        ...
        self.last_h_plan = hidden_state_plan
        ...
        act, h_dec, _ = self.decoder(inputs, discr_h, hidden_state_dec, task, mask, actions)
        # pre_state = th.cat([pre_own.unsqueeze(1), pre_enemy, pre_ally], dim=1)
...
class Decoder(nn.Module):
    ...
    def forward(self, inputs, discr_h, hidden_state, task, mask=False, actions=None):
        hidden_state = hidden_state.reshape(-1, 1, self.entity_embed_dim)
        cls_hidden = discr_h
