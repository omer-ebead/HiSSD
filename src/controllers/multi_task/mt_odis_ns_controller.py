...
class ODISNSMAC:
    ...
    def forward_qvalue(self, ep_batch, task, attn_skill, attn_obs, test_mode=False):
        agent_outs = self.agent.forward_qvalue(attn_skill, attn_obs)

        return agent_outs.view(ep_batch.batch_size, attn_skill.shape[1], self.task2n_agents[task], -1)
    ...
    def forward_obs_skill(self, ep_batch, t, task, mask_flag=False, test_mode=False):
        agent_inputs = self._build_inputs(ep_batch, t, task)
        agent_outs, self.hidden_states_enc = self.agent.forward_obs_skill(agent_inputs, self.hidden_states_enc, task, mask_flag)
        # _, n_entity, _ = agent_outs.shape

        # return agent_outs.view(ep_batch.batch_size, self.task2n_agents[task], n_entity, -1)
        return agent_outs.view(ep_batch.batch_size, self.task2n_agents[task], -1)

    # def forward_seq_action(self, ep_batch, skill_action, t, task, test_model=False):
    def forward_seq_action(self, ep_batch, t, task, attn_skill=None, mask=False, test_model=False):
        agent_seq_inputs = []
        # if t == 0:
        #     self.q_skill = None
        ...
        agent_seq_outs, skill_seq_outs, self.hidden_states_dec = self.agent.forward_seq_action(
            agent_seq_inputs, self.hidden_states_dec, task, mask, t, actions)

        return agent_seq_outs.view(
            ep_batch.batch_size, self.c_step, self.task2n_agents[task], -1), skill_seq_outs.view(
            ep_batch.batch_size, self.c_step, self.task2n_agents[task], -1)

    def forward_contrastive(self, skill):
        skill_ = self.agent.forward_contrastive(skill)
        return skill_

    def forward_global(self, skill):
        skill_ = self.agent.forward_global(skill)
        return skill_
    ...
    def forward(self, ep_batch, t, task, test_mode=False):
        ...
        if t % self.c_step == 0:
            agent_outs, self.hidden_states_enc, self.hidden_states_dec, self.skill = self.agent(
                agent_inputs, self.hidden_states_enc, self.hidden_states_dec, task, None)
        else:
            agent_outs, self.hidden_states_enc, self.hidden_states_dec, _ = self.agent(
                agent_inputs, self.hidden_states_enc, self.hidden_states_dec, task, self.skill)
    ...
    def init_hidden(self, batch_size, task):
        ...
        hidden_states_enc, hidden_states_dec, skill_hidden = self.agent.init_hidden()
