...
class MODELNLAgent(nn.Module):
    ...
    def forward_action(self, inputs, emb_inputs, hidden_state_dec, hidden_state_plan, task, 
                       mask=False, t=0, actions=None):
        h_plan = hidden_state_plan
        act, h_dec = self.decoder(emb_inputs, inputs, hidden_state_dec, task, mask, actions)
    ...
    def forward(self, inputs, hidden_state_plan, hidden_state_dec, hidden_state_dis, t, task, skill, 
        ...
        act, h_dec = self.decoder(self.last_out_h, inputs, hidden_state_dec, task, mask, actions)
        # pre_state = th.cat([pre_own.unsqueeze(1), pre_enemy, pre_ally], dim=1)
